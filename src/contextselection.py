...
class ContextSelection:
    def __init__(self, selection_strategy: str, df_path: str, column_name: str, top_k_far: int, top_k_near: int,
                 **kwargs):
    ...
    def _process_anomalies(self):
        ...
        for _, anomaly in anomalies.iterrows():
            ...
            self._process_single_anomaly(line_number, log_line)

        return self.context_df

    def _process_single_anomaly(self, line_number, log_line):
        ...
        self._get_near_context(line_number)
        self._get_far_context(line_number, log_line)

    def _get_near_context(self, line_number):
    ...
    def _get_far_context(self, line_number, log_line):
        """Add the semantically or lexically similar far context (logs with similar content)"""
        id_col = 'row_nr' if 'row_nr' in self.context_df.columns else 'LineId'
        ...
        if self.selection_strategy == "hybrid":
            semantic_similarity_matrix, success = self._semantic_matrix(reduced_df, log_line)
            similarity_matrices.append(semantic_similarity_matrix)

            if success:  # preventing calculating lexical similarity twice
                lexical_similarity_matrix = self._lexical_matrix(reduced_df, log_line)
                similarity_matrices.append(lexical_similarity_matrix)

        elif self.selection_strategy == "semantic":
            semantic_similarity_matrix, _ = self._semantic_matrix(reduced_df, log_line)
            similarity_matrices.append(semantic_similarity_matrix)
    ...
    def _semantic_matrix(self, reduced_df, log_line) -> Tuple[pd.DataFrame, bool]:
        """Calculate semantic similarity between log_line and entries in reduced_df"""
        if len(reduced_df) == 0:
            return pd.DataFrame(columns=['similarity']), False
        ...
        try:
            ...
            return self._lexical_matrix(reduced_df, log_line), False

        similarity_df = reduced_df.copy()
        similarity_df['similarity'] = cosine_similarities
        return similarity_df, True

    def _lexical_matrix(self, reduced_df, log_line) -> pd.DataFrame:
        """Calculate lexical similarity between log_line and entries in reduced_df"""
        if len(reduced_df) == 0:
    ...
    def get_context(self, provided_df=None):
        """
        Returns the context for log entries.
        
        Args:
            provided_df: Optional DataFrame to use instead of loading from file
            
        Returns:
            DataFrame with context_ids_ref column added
        """
        ...
        result_df = self._process_anomalies()

        # Convert string context_ids to references to LineId in context_ids_ref column
        result_df['context_ids_ref'] = result_df['context_ids']
